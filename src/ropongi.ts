...
export class Ropongi {
    ...
    schedules = [] as any;
    schedulesGenres = [ [] as any, [] as any, [] as any, [] as any, [] as any, [] as any, [] as any ];
    schedulesGenresSpliters = [ [] as any, [] as any, [] as any, [] as any, [] as any, [] as any, [] as any ];
    schedulesStart = [] as any;
    schedulesStartObject = [] as any;
    schedulesStop = [] as any as any;
    schedulesStopObject = [] as any;
            ...
            switch (line.trim().split(' ')[0]) {
                ...
                case 'make':
                    switch (line.trim().split(' ')[1]) {
                        case 'playlist':
                            ...
                            if (arr.length > 0 && arr.length < 3) this.createPlayListSwitch(arr, false, false);
                            else this.logAndPrint('fail', 'input must be 1 parameter.');
                            break;
                        case 'playlist+shared':
                            arr = line.trim().split(' ').slice(2);
                            if (arr.length > 0 && arr.length < 3) this.createPlayListSwitch(arr, false, true);
                            else this.logAndPrint('fail', 'input must be 1 parameter.');
                            break;
                        case 'random':
                            switch (line.trim().split(' ')[2]) {
                                case 'playlist':
                                    ...
                                    if (arr.length > 0 && arr.length < 3) this.createPlayListSwitch(arr, true, false);
                                    else this.logAndPrint('fail', 'input must be 1 parameter.');
                                    break;
                            }
                            break;
                        case 'random+shared':
                            switch (line.trim().split(' ')[2]) {
                                case 'playlist':
                                    arr = line.trim().split(' ').slice(3);
                                    if (arr.length > 0 && arr.length < 3) this.createPlayListSwitch(arr, true, true);
                                    else this.logAndPrint('fail', 'input must be 1 parameter.');
                                    break;
                            }
                            break;
                ...
                case 'del':
                    switch (line.trim().split(' ')[1]) {
                        ...
                        case 'task':
                            arr = line.trim().split(' ').slice(2);
                            if (arr.length === 1) this.delSchedule(arr[0]);
                            else this.logAndPrint('fail', 'input must be 1 parameter.');
                            break;
                        default:
                            this.logAndPrint('fail', line.trim() + ' bad command, use help for list of commands.');
                    ...
                    switch (line.trim().split(' ')[1]) {
                        ...
                        case 'email':
                            {
                                ...
                                if (arr.length === 2) this.setEmail(arr[0], arr[1]);
                                else this.logAndPrint('fail', 'input must be 2 parameters (email pass).'.bold);
                                break;
                            }
                        case 'task':
                            {
                        ...
                        case 'passport':
                            ...
                            case 'output':
                                {
                                    let data = line.trim().split(' ').slice(2)[0];
                                    if (!this.setConfigs('output', data)) this.logAndPrint('fail', line.trim() + ' bad command, use help for list of commands.');
                                    break;
                                }
                        case 'wificheck':
                           { let field = line.trim().split(' ')[2],
    ...
    checkWifi() {
        this.exec('sudo ifconfig wlan0', (error: any, stdout: string | string[], stderr: any) => {
            if (stdout && stdout.indexOf('inet addr:') === -1) {
                this.exec('sudo ifdown --force wlan0', (error: any, stdout: any, stderr: any) => {
                    setTimeout(()  => {
                        this.exec('sudo ifup --force wlan0', (error: any, stdout: any, stderr: any) => {
    ...
    updateToRTC(cb = null) {
        if (this.rtc) {
            this.exec('sudo hwclock -w', (err: any, stdout: any, stderr: any) => {
                if (!err) {
    ...
    updateFromRTC(cb: { (err: any): void; (err: any): void; (err: any): void; (err: any): void; (arg0: { err: string; }): void; }) {
        if (this.rtc) {
            this.exec('sudo hwclock -s', (err: any, stdout: any, stderr: any) => {
                if (!err) {
    ...
    enableRTC(cb: undefined) {
        ...
        this.exec('sudo echo ds1307 0x68 > /sys/class/i2c-adapter/i2c-1/new_device', (err: { code: number; }, stdout: any, stderr: any) => {
    ...
    setTimeManual(millis: string) {
        if (millis && millis.toString().length === 13) {
            clearTimeout(this.setTimeTimeout);
            this.exec('sudo date --set="' + new Date(parseInt(millis)) + '"', (err: any, stdout: any, stderr: any) => {
                if (stdout) {
    ...
    setTimeProxy() {
        ...
        this.http.get(proxy, (res: any) => {
            let stringData = '';
            res.on("data", (rdata: string)  => {
                stringData += rdata;
            });
            res.on('end', () => {
                let millis = this.getMillis(stringData);
                if (millis) {
                    this.exec('sudo date --set="' + new Date(millis) + '"', (err: any, stdout: any, stderr: any) => {
                        if (stdout) {
                            this.logAndPrint('info', 'time set to: ' + new Date());
                            this.updateToRTC();
                            this.eventEmitter.emit('timeSet');
    ...
    logError(data: string) {
        ...
        if (this.configs.logs) this.fs.appendFile(path + '/' + fileName, 'command: ' + data + '\n', (err: string) => {
    ...
    logInput(input: string) {
        ...
        if (this.configs.logs) this.fs.appendFile(path + '/' + fileName, 'command: ' + input + '\n', (err: string) => {
            if (err) console.log('info: '.red + '(' + this.getTime() + ') ' + 'failing to write log, ' + err);
        });
    }
    
    logAndPrint(type: string, output: any) {
        if (type === 'pass') console.log('pass: '.green + '(' + this.getTime() + ') ' + output);
        else if (type === 'passInfo') console.log('pass: '.cyan + '(' + this.getTime() + ') ' + output);
        else if (type === 'info') console.log('info: '.cyan + '(' + this.getTime() + ') ' + output);
        else if (type === 'warningInfo') console.log('info: '.red + '(' + this.getTime() + ') ' + output);
        else if (type === 'fail') console.log('fail: '.red + '(' + this.getTime() + ') ' + output);
        else if (type === 'err') console.log('err: '.red + '(' + this.getTime() + ') ' + output);
        let path = __dirname + '/logs',
            fileName = this.getDate() + '.log';
        if (this.configs.logs) this.fs.appendFile(path + '/' + fileName, type + ': (' + this.getTime() + ') ' + output + '\n', (err: string) => {
            if (err) console.log('info: '.red + '(' + this.getTime() + ') ' + 'failing to write log, ' + err);
        });
    }
    ...
    delGenres(genresArr: any[]) {
        ...
        for (let i = genresArrLength; i > 0; i--) {
            ...
            } else if (this.genres.indexOf(genresArr[i - 1]) !== -1) deletedGenres += genresArr[i - 1] + ' ';
        }
        if (this.lastGenresPlays.length === 0) this.loadLastGenresPlays();
        let modifed = false;
        for (let i in genresArr) {
            this.delGenrePlaylist(genresArr[i]);
        }
        this.saveLastGenresPlays();
        if (modifed) this.saveLastGenresPlays();
        this.genres = this._l.difference(this.genres, genresArr);
        this.saveGenres();
        deletedGenres.trim();
        usedGenres.trim();
        if (usedGenres.length !== 0) this.logAndPrint('passInfo', 'genres in use: ' + usedGenres);
        ...
        if (this.fs.existsSync(__dirname + '/saves/genres.json')) {
            try {
                ...
                for (let i in this.genres)
                    if (!this.fs.existsSync(__dirname + '/uploads/genres/' + this.genres[i])) this.fs.mkdirSync(__dirname + '/uploads/genres/' + this.genres[i]);
            } catch (err) {
    ...
    saveLastPlay() {
        if (this.isDaysMode()) {
            ...
            if (!modifed) this.lastGenresPlays.push({
                files: this.playlist.files,
                currentIndex: this.playlist.currentIndex,
                directory: this.playlist.directory
            });
        ...
        if (!this.omx.isPlaying() && !forceStop) {
            ...
            this.omx.once('end', () => {
                if (this.new_rdm_at_end === 1 && this.streaming && this.playlist.currentIndex === 0 ){
                    ...
                    this.exec('sudo killall omxplayer', (err: any, stdout: any, stderr: any) => {
                        if (!err) {
    ...
    delPlayListSwitch(arrDayGenre: string | any[]) {
        if (arrDayGenre.length === 2 && this.isAllString(arrDayGenre[0])) {
            ...
            else if (arrDayGenre[1] === 'genres') this.delGenresPlayList(arrDayGenre[0]);
        } else if (arrDayGenre.length === 1 && !this.isAllString(arrDayGenre[0])) {
            if (this.isRealDayName(arrDayGenre[0])) this.delPlayList(arrDayGenre[0]);
    ...
    delPlayList(day: string) {
        if (this.isRealDayName(day)) {
            ...
            if (this.fs.existsSync(daydir + '/_playlist.m3u')) {
                ...
                return true;
            } else this.logAndPrint('fail', day + ' missing a playlist.');
    ...
    createPlayList(day: string, random: any, shared: any) {
        ...
        for (i; i < endi; i++) {
            ...
            if (this.fs.existsSync(daydir)) {
                ...
                if (shared) sharedFiles = this.getOnlyPlayFiles(this.fs.readdirSync(this.sharedday + '/'));
                if (this.util.isArray(sharedFiles)) files = files.concat(sharedFiles);
                if (this.util.isArray(files)) {
                    if (files.length === 0) this.logAndPrint('fail', 'no files exists in ' + this.weekday[i]);
                    else {
                        if (!this.fs.existsSync(daydir + '/_playlist.m3u')) {
                            if (random)
                            ...
                            for (j = 0; j < files.length; j++) this.fs.appendFileSync(daydir + '/_playlist.m3u', files[j] + '\n');
                            if (this.fs.existsSync(daydir + '/_playlist.m3u')) this.logAndPrint('pass', 'new playlist created in ' + this.weekday[i]);
                        } else this.logAndPrint('fail', 'playlist already exists in ' + this.weekday[i]);
                    }
                }
            }
        }
    ...
    addSchedule() {
        ...
        if (this.isGenresMode()) this.logAndPrint('passInfo', 'use: set taskgenres ' + this.schedules[startDayIndex].startDay + ' GENRE or GENRE TIME GENRE... GENRE');
    ...
    currentStatus() {
        ...
        this.exec('vcgencmd measure_temp', (error: any, stdout: { toString: () => string; }, stderr: any) => {
            if (stdout) {
                this.logAndPrint('pass', 'temp: ' + stdout.toString().replace(/\s+/g, " ").trim().split('=')[1]);
            }
        });
        this.exec('df / -h', (error: any, stdout: { toString: () => string; }, stderr: any) => {
            if (stdout) {
    ...
    getListOfGenresDirs() {
        let path = __dirname + '/uploads/genres',
            dirs: any[] = [],
            files = [];
        try {
        ...
        for (let i in files) {
            if (this.fs.statSync(path + '/' + files[i]).isDirectory()) dirs.push(files[i]);
        }
        return dirs;
    }
    ...
    printGenres() {
        ...
        if (this.genres.length !== 0) {
            for (let i in this.genres) {
                let genrePlaylist = this.getLastGenresPlaysPlaylist(this.genres[i]),
                    extraInfo = '';
    ...
    delGenresPlayList(genre: string) {
        if (this.isRealGenre(genre)) {
            ...
            if (this.fs.existsSync(playdir + '/_playlist.m3u')) {
                ...
                return true;
            } else this.logAndPrint('fail', genre + ' is missing a playlist.');
    ...
    createGenresPlayList(genre: string, random: boolean) {
        ...
        let i = 0,
            ...
            genredir,
            files = [],
            genres = this.getListOfGenresDirs(),
            rNum, fileName;
        if (this.isRealGenre(genre)) {
        ...
        for (i; i < endi; i++) {
            ...
            if (this.fs.existsSync(genredir)) {
                ...
                if (this.util.isArray(files)) {
                    if (files.length === 0) this.logAndPrint('fail', 'no files exists in genre ' + genres[i]);
                    else {
                        if (!this.fs.existsSync(genredir + '/_playlist.m3u')) {
                            ...
                            for (j = 0; j < files.length; j++) this.fs.appendFileSync(genredir + '/_playlist.m3u', files[j] + '\n');
                            if (this.fs.existsSync(genredir + '/_playlist.m3u')) this.logAndPrint('pass', 'new playlist created in ' + genres[i]);
                            let playlist = {
                                files: files,
                                currentIndex: 0,
                                directory: genres[i]
                            }
                            if (this.updateLastGenresPlays(playlist)) this.saveLastGenresPlays();
                        } else this.logAndPrint('fail', 'playlist already exists in ' + genres[i]);
                    }
                }
            }
        }
    ...
    buildTimeLineGenres(day: string) {
        ...
        let dayIndex = this.weekday.indexOf(day),
            timeline = '';
        if (this.schedulesGenres[dayIndex].length === 0) timeline += 'empty.'.red;
        else {
            ...
            for (let i in this.schedulesGenres[dayIndex]) {
                ...
                if (i < this.schedulesGenresSpliters[dayIndex].length) {
                    if (this.is48Hours(this.schedulesGenresSpliters[dayIndex][i])) {
                        ...
                        timeline += ') ';
                    } else timeline += !this.is24Hours(this.schedulesGenresSpliters[dayIndex][i]) ?this.make24Hours(this.schedulesGenresSpliters[dayIndex][i]) : this.schedulesGenresSpliters[dayIndex][i] + ' ';
                }
            }
            if (this.schedules[dayIndex].startDay !== this.schedules[dayIndex].endDay) timeline += '(' + this.schedules[dayIndex].endDay + ' ' + this.schedules[dayIndex].endTime + ')';
            else timeline += this.schedules[dayIndex].endTime;
        }
    ...
    getCurrentGenre(day: string) {
        let dayIndex = this.weekday.indexOf(day),
            currentTime = this.schedules[dayIndex].startDayIndex !== this.schedules[dayIndex].endDayIndex 
                && this.schedules[dayIndex].endDayIndex === this.updateDay().index ? this.make48Hours(this.getTime()) : this.getTime(),
            genreIndex = 0,
            chr, cm, shr, sm;
    ...
    validateSceduleTimes(day: string, times: any[]) {
        ...
        let dayIndex = this.weekday.indexOf(day),
            endTime = dayIndex !== this.schedules[dayIndex].endDayIndex ? this.make48Hours(this.schedules[dayIndex].endTime) : this.schedules[dayIndex].endTime,
            limitTime = this.schedules[dayIndex].startTime,
    ...
    shutdownIfSet() {
        if (this.configs.autoShutdown) {
            ...
            this.exec('shutdown -h now', (error: string, stdout: any, stderr: string) => {
                if (stderr) this.logAndPrint('warningInfo', 'stderr from auto shutdown: ' + stderr);
                if (error) this.logAndPrint('warningInfo', 'error from auto shutdown: ' + error);
            });
        }
    }
